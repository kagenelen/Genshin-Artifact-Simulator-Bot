...
DOMAIN_FODDER = 54095 # Actual average is 10735

####################### Don't write code above here ##############################
"""
TODO List:
- actual exceptions
- daily domain run limit
- reroll cost currency (for now reroll will deduct)
"""
####################### Don't write code above here ##############################
...
@bot.command(name="inventory", 
			 brief="View inventory. E.g: ?inventory [opt: max/lock/sandbox/reroll]",
			aliases=["i", "inv"])
...
@bot.command(name="lock", 
			 brief="Lock/unlock an artifact. E.g: ?lock [artifact id]",
...
@bot.command(name="sandbox", 
			 brief="Create custom artifact. E.g: ?sandbox \"[set]\" [type] [mainstat] [substat1] ...",
			aliases=["s", "custom"])
...
- Clear pool and mountain cavern: bloodstained chivalry (BC), noblesse oblige (NO)
- Domain of guyun: retracing bolide (RB), archaic petra (AP)
- Hidden palace of zhou formula: crimson witch of flames (CWoF), lavawalker (LW)
- Midsummer courtyard: thundering fury (TF), thundersoother (TS)
- Momiji-dyed court: emblem of severed fate (EoSF), shimenawa's reminiscence (SR)
- Peak of vindagnyr: blizzard strayer (BS), heart of depth (HoD)
- Spire of solitary enlightenment: deepwood memories (DM), gilded dreams (GD)
- Valley of remembrance: viridescent venerer (VV), maiden beloved (MB)
...
**Difference between beta and completed version.**
In beta you can have unlimited domain runs. However, inventory are subject to be cleared anytime. After beta (these are subject to change), this will be limited to 20 runs per day. There will be ways to get more runs after reaching the daily limit. And rerolls won't be free.
...
Either you found a bug or you didn't put multi-word arguments in quotations. E.g. ?domain "valley of remembrance" will work.

**Will more domains be added?**
